...
class App:
    ...
    def base_where(self):
        ...
        if term:
            return f"WHERE {COLUMN} LIKE ?", (f"%{term}%",)
        return "", ()

    def order_clause(self):
        if self.sort_mode == "alpha":
            return f"ORDER BY {COLUMN} COLLATE NOCASE"
        return "ORDER BY rowid"

    def query_total(self):
        where_sql, params = self.base_where()
    ...
    def load_page(self):
        ...
        self.status.config(
            text=f"{start}-{end} of {total} | sort: {sort_label}" + (f" | search: {term}" if term else "")
        )
