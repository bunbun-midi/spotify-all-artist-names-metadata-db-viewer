...
class App:
    def __init__(self, root):
        ...
        self.subquery_var = tk.StringVar()
    ...
    def build_ui(self):
        search_block = ttk.Frame(self.root, padding=8)
        search_block.pack(fill="x")

        top = ttk.Frame(search_block)
        top.pack(fill="x")

        ttk.Label(top, text="Search:", width=10).pack(side="left")
        ...
        sub = ttk.Frame(search_block)
        sub.pack(fill="x", pady=(6, 0))

        ttk.Label(sub, text="Subquery:", width=10).pack(side="left")
        ttk.Entry(sub, textvariable=self.subquery_var, width=35).pack(side="left", padx=6)
        ttk.Button(sub, text="Search", command=self.do_subquery).pack(side="left")
        ttk.Button(sub, text="Clear", command=self.clear_subquery).pack(side="left", padx=4)

        self.subquery_result = ttk.Label(search_block, text="", wraplength=900, justify="left")
        self.subquery_result.pack(fill="x", pady=(6, 0))
        ...
        self.table_frame = ttk.Frame(self.root, padding=8)
        self.table_frame.pack(fill="both", expand=True)
        self.table_frame.bind("<Configure>", self.on_table_configure)

        self.tree = ttk.Treeview(self.table_frame, columns=(COLUMN,), show="headings", height=20)
        self.tree.heading(COLUMN, text=COLUMN)
        self.tree.column(COLUMN, width=800, anchor="w", stretch=True)
    ...
    def on_root_configure(self, event):
        if event.widget is self.root:
            self.subquery_result.configure(wraplength=max(event.width - 16, 100))

    def on_table_configure(self, event):
        if event.widget is not self.table_frame:
            return
        rowheight = 24
        heading = 28
        rows = max((event.height - heading) // rowheight, 5)
        if rows != self.tree.cget("height"):
    ...
    def refresh(self):
        ...
        status = f"{start}-{end} of {total} | sort: {sort_label}"
        if term:
            status += f" | search: {term}"
    ...
    def do_search(self):
        self.page = 0
        self.subquery_result.config(text="")
        self.refresh()
    ...
    def do_subquery(self):
        sub = self.subquery_var.get().strip()
        if not sub:
            messagebox.showerror("Subquery", "Enter a subquery.")
            return
        ...
        cur.execute(
            f"""
            SELECT artist_name, rn FROM (
                SELECT {COLUMN} AS artist_name,
        ...
        found = cur.fetchall()
        if not found:
            self.subquery_result.config(text=f"No match for '{sub}' in the current results.")
            return

        parts = [
            f"{row['artist_name']} (page {((row['rn'] - 1) // PAGE_SIZE) + 1})"
            for row in found
        ]
        self.subquery_result.config(text="; ".join(parts))

    def clear_subquery(self):
        self.subquery_var.set("")
        self.subquery_result.config(text="")
